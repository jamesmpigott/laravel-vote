<template>
    <form action="#" @submit.prevent="submit()" v-if="pollFetched">
        <div v-if="success" class="flex flex-col items-center justify-center text-green-500">
            ...
            <span class="block text-lg font-semibold text-slate-900">{{ this.$trans.get('generic.vote-submitted') }}</span>
        </div>
        <div v-else class="text-left">
            ...
            <div class="mb-8">
                <div class="form-check my-2.5" v-for="(option, index) in poll.options" :key="'option_' + index">
            ...
            <div v-if="!poll.anon_voting" class="flex items-start flex-col">
                <label for="email" class="text-sm text-gray-600">{{ this.$trans.get('generic.email') }}</label>
